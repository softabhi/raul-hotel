...
import { fetchRoomById, createBooking } from "@/data/api";
...
function BookingContent() {
  ...
  useEffect(() => {
    if (roomId) {
      fetchRoomById(roomId).then(setRoom);
    }
  }, [roomId]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#FAFAF8] px-6 pt-24">
        <div className="max-w-md w-full text-center bg-white rounded-3xl shadow-2xl p-10 border border-gray-100">
          ...
          <div className="bg-[#FAFAF8] rounded-xl p-4 text-sm text-left mb-6 border border-gray-100">
            <p><strong>Booking ID:</strong> {bookingResponse?.booking_id || `LXS-${100000 + (bookingResponse?.id || 0)}`}</p>
            ...
            {step === 3 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setStep(2)} className="btn-outline-gold flex-1 justify-center" disabled={isSubmitting}>Back</button>
                  <button 
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (res.success) {
                          ...
                          alert(res.message || "Failed to place booking");
                        }
                      } catch (err) {
                  ...
                  >
                    {isSubmitting ? "Processing..." : `Confirm & Pay ₹${total.toLocaleString()}`}
                  </button>
                </div>
              </div>
            )}
